...
# 焦点距離を返す
def returnFocus_Lens45re(paramList):
    Nlens4 = paramList[0][0]
    NBlueRay4 = paramList[0][1]
    Nlens5 = paramList[1][0]
    NBlueRay5 = paramList[1][1]
    ...
    for i in raySPoint0:
        ...
        T = VF.rayTraceDecideT_Lens5R(raySPoint0, directionVector0)
        rayEPoint0 = raySPoint0 + T*directionVector0
        #VF.plotLinePurple(raySPoint0, rayEPoint0)  # 入射光描画

        # 赤色光
        refractionV = directionVector0

        refractSPoint_Lens5R = rayEPoint0  # 以下、レンズ２についての計算
        normalV_Lens5R = VF.decideNormalV_Lens5R(refractSPoint_Lens5R)
        refractionV_Lens5R = VF.decideRefractionVL(refractionV, normalV_Lens5R, Nair, Nlens5)
        T = VF.rayTraceDecideT_Lens5L(refractSPoint_Lens5R, refractionV_Lens5R)
        refractEPoint_Lens5R = refractSPoint_Lens5R + T*refractionV_Lens5R
        #VF.plotLineRed(refractSPoint_Lens5R,refractEPoint_Lens5R)  # 屈折光の描画
        raySPoint_Lens5L = refractEPoint_Lens5R
        normalV_Lens5L = VF.decideNormalV_Lens5L(raySPoint_Lens5L)
        refractionV_Lens5L = VF.decideRefractionVR(refractionV_Lens5R, normalV_Lens5L, Nlens4, Nlens5)
        T = 0  # レンズの接着を考えた
        rayEPoint_Lens4R = raySPoint_Lens5L + T*refractionV_Lens5L
        #VF.plotLineRed(raySPoint_Lens5L, rayEPoint_Lens4R)

        refractSPoint_Lens4R = rayEPoint_Lens4R  # 以下、レンズ1についての計算
        normalV_Lens4R = VF.decideNormalV_Lens4R(refractSPoint_Lens4R)
        ...
        T = VF.rayTraceDecideT_Screen(raySPoint_Lens4L, refractionV_Lens4L)
        rayEPoint_Last = raySPoint_Lens4L + T*refractionV_Lens4L
        #VF.plotLineRed(raySPoint_Lens4L, rayEPoint_Last)


        T = T_FocusGraph(raySPoint_Lens4L, refractionV_Lens4L)
        focusPoint = raySPoint_Lens4L + T*refractionV_Lens4L
        RedFocusPoints=focusPoint[0]

        T = T_FocalLength(refractSPoint_Lens5R, refractionV_Lens4L)
        principalPoint = focusPoint - T*refractionV_Lens4L
        ...
        refractSPoint_Lens5R = rayEPoint0  # 以下、レンズ２についての計算
        normalV_Lens5R = VF.decideNormalV_Lens5R(refractSPoint_Lens5R)
        refractionV_Lens5R = VF.decideRefractionVL(refractionV, normalV_Lens5R, Nair, Nlens5*NBlueRay5)
        T = VF.rayTraceDecideT_Lens5L(refractSPoint_Lens5R, refractionV_Lens5R)
        refractEPoint_Lens5R = refractSPoint_Lens5R + T*refractionV_Lens5R
        #VF.plotLineBlue(refractSPoint_Lens5R,refractEPoint_Lens5R)  # 屈折光の描画
        raySPoint_Lens5L = refractEPoint_Lens5R
        normalV_Lens5L = VF.decideNormalV_Lens5L(raySPoint_Lens5L)
        refractionV_Lens5L = VF.decideRefractionVR(refractionV_Lens5R, normalV_Lens5L, Nlens4*NBlueRay4, Nlens5*NBlueRay5)
        T = 0  # レンズの接着を考えた
        rayEPoint_Lens4R = raySPoint_Lens5L + T*refractionV_Lens5L
        #VF.plotLineBlue(raySPoint_Lens5L, rayEPoint_Lens4R)
        ...
        T = VF.rayTraceDecideT_Screen(raySPoint_Lens4L, refractionV_Lens4L)
        rayEPoint_Last = raySPoint_Lens4L + T*refractionV_Lens4L
        #VF.plotLineBlue(raySPoint_Lens4L, rayEPoint_Last)
...
for i in Params:
    plotLens(*i)
returnFocus_Lens45re([[1.61503, 1.00604],[1.48534, 1.00465]])
