...
def MacroLens_reverse(startX=10.695, startZ=0):
    ...
    # スクリーン描画
    Ys, Zs = np.meshgrid(
            np.arange(-3.5, 3.5, 0.2),
            np.arange(-2.8, 2.8, 0.2))
    Xs = 0*Ys + 0*Zs + -10
    ax.plot_wireframe(Xs, Ys, Zs, linewidth=0.2, color='k')
    ...
    pointsY, pointsZ = np.meshgrid(
        np.arange(-width+centerY, 1+width+centerY, space),
        np.arange(-width+centerZ, 1+width+centerZ, space))
    pointsX = np.array([centerX*0]*size) + lens1V[0]*0
    pointsY = pointsY.reshape(size)*rayDensity + lens1V[1]*0
    pointsZ = pointsZ.reshape(size)*rayDensity + lens1V[2]*0
    directEnd = VF.makePoints(pointsX, pointsY, pointsZ, size, 3)

    raySPoint = np.array([startX, 0, startZ])  # 始点

    directV = directEnd - raySPoint

    for i in directV:
        #directionVector0 = np.array([1, 0, 0])  # 入射光の方向ベクトルを設定
        directionVector5R = i
        T = VF.rayTraceDecideT_Lens5R(raySPoint, directionVector5R)  # 交点のための係数
        rayEPoint5R = raySPoint + T*directionVector5R  # 入射光の終点
        VF.plotLinePurple(raySPoint, rayEPoint5R)  # 入射光描画

        # 赤色光
        refractSPoint5R = rayEPoint5R  # 入射光の終点を引き継ぐ。以下レンズ５についての計算
        normalV_Lens5R = VF.decideNormalV_Lens5R(refractSPoint5R)  # レンズの法線を求める
        # 屈折光の方向ベクトルを求める
        refractionV_Lens5R = VF.decideRefractionVL(directionVector5R, normalV_Lens5R, Nair, Nlens5)
        # 係数Tを求めて、屈折光の終点も求める
        T = VF.rayTraceDecideT_Lens5L(refractSPoint5R, refractionV_Lens5R)
        refractEPoint5L = refractSPoint5R + T*refractionV_Lens5R
        VF.plotLineRed(refractSPoint5R,refractEPoint5L)  # 屈折光の描画
        raySPoint5L = refractEPoint5L  # 屈折光の終点を引き継ぐ
        normalV5L = VF.decideNormalV_Lens5L(raySPoint5L)  # レンズの法線を求める
        # 屈折光の方向ベクトルを求める
        refractionV_Lens5L = VF.decideRefractionVR(refractionV_Lens5R, normalV5L, Nlens4, Nlens5)
        T = 0  # レンズの接着を考えた
        rayEPoint5L = raySPoint5L + T*refractionV_Lens5L  # 空気中の屈折光の終点
        VF.plotLineRed(raySPoint5L,rayEPoint5L)  # 空気中の屈折光の描画

        refractSPoint_Lens4R = rayEPoint5L  # 以下、レンズ４についての計算
        normalV_Lens4R = VF.decideNormalV_Lens4R(refractSPoint_Lens4R)  # レンズの法線を求める
        # 屈折光の方向ベクトルを求める
        refractionV_Lens4R = VF.decideRefractionVL(refractionV_Lens5L, normalV_Lens4R, Nlens5, Nlens4)
        # 係数Tを求めて、屈折光の終点も求める
        T = VF.rayTraceDecideT_Lens4L(refractSPoint_Lens4R, refractionV_Lens4R)
        refractEPoint_Lens4R = refractSPoint_Lens4R + T*refractionV_Lens4R
        VF.plotLineRed(refractSPoint_Lens4R, refractEPoint_Lens4R)  # 屈折光の描画
        raySPoint_Lens4L = refractEPoint_Lens4R
        normalV_Lens4L = VF.decideNormalV_Lens4L(raySPoint_Lens4L)
        refractionV_Lens4L = VF.decideRefractionVR(refractionV_Lens4R, normalV_Lens4L, Nair, Nlens4)
        T = VF.rayTraceDecideT_Lens3R(raySPoint_Lens4L, refractionV_Lens4L)
        rayEPoint_Lens3R = raySPoint_Lens4L + T*refractionV_Lens4L
        VF.plotLineRed(raySPoint_Lens4L, rayEPoint_Lens3R)

        # 絞り
        rayEPoint_Lens3R_TEST = raySPoint_Lens4L + -0.83*refractionV_Lens4L
        TestR = rayEPoint_Lens3R_TEST[1]**2 + rayEPoint_Lens3R_TEST[2]**2
        if TestR >= 1:
            refractionV_Lens4L = [0,0,0]

        refractSPoint_Lens3R = rayEPoint_Lens3R  # 以下、レンズ３についての計算
        normalV_Lens3R = VF.decideNormalV_Lens3R(refractSPoint_Lens3R)  # レンズの法線を求める
        # 屈折光の方向ベクトルを求める
        refractionV_Lens3R = VF.decideRefractionVL(refractionV_Lens4L, normalV_Lens3R, Nair, Nlens3)
        # 係数Tを求めて、屈折光の終点も求める
        T = VF.rayTraceDecideT_Lens3L(refractSPoint_Lens3R, refractionV_Lens3R)
        refractEPoint_Lens3R = refractSPoint_Lens3R + T*refractionV_Lens3R
        VF.plotLineRed(refractSPoint_Lens3R,refractEPoint_Lens3R)  # 屈折光の描画
        raySPoint_Lens3L = refractEPoint_Lens3R
        normalV_Lens3L = VF.decideNormalV_Lens3L(raySPoint_Lens3L)
        refractionV_Lens3L = VF.decideRefractionVR(refractionV_Lens3R, normalV_Lens3L, Nair, Nlens3)
        T = VF.rayTraceDecideT_Lens2R(raySPoint_Lens3L, refractionV_Lens3L)
        rayEPoint_Lens2R = raySPoint_Lens3L + T*refractionV_Lens3L
        VF.plotLineRed(raySPoint_Lens3L, rayEPoint_Lens2R)

        refractSPoint_Lens2R = rayEPoint_Lens2R  # 以下、レンズ２についての計算
        ...
        refractSPoint_Lens1R = rayEPoint_Lens1R  # 以下、レンズ5についての計算
        ...
        normalV_Lens5R = VF.decideNormalV_Lens5R(refractSPoint5R)  # レンズの法線を求める
        # 屈折光の方向ベクトルを求める
        refractionV_Lens5R = VF.decideRefractionVL(directionVector5R, normalV_Lens5R, Nair, Nlens5*NBlueRay5)
        # 係数Tを求めて、屈折光の終点も求める
        T = VF.rayTraceDecideT_Lens5L(refractSPoint5R, refractionV_Lens5R)
        refractEPoint5L = refractSPoint5R + T*refractionV_Lens5R
        VF.plotLineBlue(refractSPoint5R,refractEPoint5L)  # 屈折光の描画
        raySPoint5L = refractEPoint5L  # 屈折光の終点を引き継ぐ
        normalV5L = VF.decideNormalV_Lens5L(raySPoint5L)  # レンズの法線を求める
        # 屈折光の方向ベクトルを求める
        refractionV_Lens5L = VF.decideRefractionVR(refractionV_Lens5R, normalV5L, Nlens4*NBlueRay4, Nlens5*NBlueRay5)
        T = 0  # レンズの接着を考えた
        rayEPoint5L = raySPoint5L + T*refractionV_Lens5L  # 空気中の屈折光の終点
        VF.plotLineBlue(raySPoint5L,rayEPoint5L)  # 空気中の屈折光の描画

        refractSPoint_Lens4R = rayEPoint5L  # 以下、レンズ４についての計算
        normalV_Lens4R = VF.decideNormalV_Lens4R(refractSPoint_Lens4R)  # レンズの法線を求める
        # 屈折光の方向ベクトルを求める
        refractionV_Lens4R = VF.decideRefractionVL(refractionV_Lens5L, normalV_Lens4R, Nlens5*NBlueRay5, Nlens4*NBlueRay4)
        # 係数Tを求めて、屈折光の終点も求める
        T = VF.rayTraceDecideT_Lens4L(refractSPoint_Lens4R, refractionV_Lens4R)
        refractEPoint_Lens4R = refractSPoint_Lens4R + T*refractionV_Lens4R
        VF.plotLineBlue(refractSPoint_Lens4R, refractEPoint_Lens4R)  # 屈折光の描画
        raySPoint_Lens4L = refractEPoint_Lens4R
        normalV_Lens4L = VF.decideNormalV_Lens4L(raySPoint_Lens4L)
        refractionV_Lens4L = VF.decideRefractionVR(refractionV_Lens4R, normalV_Lens4L, Nair, Nlens4*NBlueRay4)
        T = VF.rayTraceDecideT_Lens3R(raySPoint_Lens4L, refractionV_Lens4L)
        rayEPoint_Lens3R = raySPoint_Lens4L + T*refractionV_Lens4L
        VF.plotLineBlue(raySPoint_Lens4L, rayEPoint_Lens3R)
        ...
        normalV_Lens3R = VF.decideNormalV_Lens3R(refractSPoint_Lens3R)  # レンズの法線を求める
        # 屈折光の方向ベクトルを求める
        refractionV_Lens3R = VF.decideRefractionVL(refractionV_Lens4L, normalV_Lens3R, Nair, Nlens3*NBlueRay3)
        # 係数Tを求めて、屈折光の終点も求める
        T = VF.rayTraceDecideT_Lens3L(refractSPoint_Lens3R, refractionV_Lens3R)
        refractEPoint_Lens3R = refractSPoint_Lens3R + T*refractionV_Lens3R
        VF.plotLineBlue(refractSPoint_Lens3R,refractEPoint_Lens3R)  # 屈折光の描画
        raySPoint_Lens3L = refractEPoint_Lens3R
        normalV_Lens3L = VF.decideNormalV_Lens3L(raySPoint_Lens3L)
        refractionV_Lens3L = VF.decideRefractionVR(refractionV_Lens3R, normalV_Lens3L, Nair, Nlens3*NBlueRay3)
        T = VF.rayTraceDecideT_Lens2R(raySPoint_Lens3L, refractionV_Lens3L)
        rayEPoint_Lens2R = raySPoint_Lens3L + T*refractionV_Lens3L
        VF.plotLineBlue(raySPoint_Lens3L, rayEPoint_Lens2R)
...
def MakeFocusGraph(Nlens1=1.60008, Nlens2=1.49552, Nlens3=1.61506, Nlens4=1.50592, Nlens5=1.58619,
    ...
    # 始点を生成する
    pointsZ = np.arange(0.001, 1.79, 0.1) + lens1V[2]
    ...
    for i in raySPoint0:
        ...
        T = T_FocusGraph(raySPoint_Lens5R, refractionV_Lens5R)
        focusPoint = raySPoint_Lens5R + T*refractionV_Lens5R
        BlueFocusPoints.append(focusPoint[0])

        T = T_FocalLength(raySPoint0, refractionV_Lens5R)
        principalPoint = focusPoint - T*refractionV_Lens5R
        BluePrincipalPoints.append(principalPoint[0])
        VF.plotLineBlack(focusPoint+[0,1,0], principalPoint+[0,1,0])
        VF.plotLineBlack(raySPoint0+[0,1,0], principalPoint+[0,1,0])
        VF.plotLineBlack(focusPoint+[0,1,0], rayEPoint_Last+[0,1,0])
